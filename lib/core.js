...
module.exports = function remotable (options) {
    ...
    function decorate (target, key, descriptor) {
        ...
        function proxy() {
            ...
            var channelingFn = onproxy (opts, methodId, func, this, args);
...
remotable.handle = (text, backchannel) => {
    respondTo (parse(text, backchannel), msg => backchannel(stringify(msg, backchannel)));
}

remotable.decide = p => proxies.push(p);

function onproxy (options, methodId, func, thiz, args) => {
    var i = proxies.length;
    while (i--) {
        var rv = proxies[i](options, methodId, func, thiz, args);
        if (rv) return rv;
    }
};
