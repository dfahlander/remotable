...
remotable.unregisterType = function (typeId) {
    replacers = replacers.filter(function (r) {return r.typeId !== typeId;});
...
remotable.configure = function (configuration) {
    Object.keys(configuration.roles).forEach(function (role) {
        roles[role] = configuration.roles[role];
    });
